...
3. **GraphRAG**：WHO、AHA/ASA、ADA、KDIGO、随机试验和系统综述等 18 份原始知识源 + 42 条分层注册摘要，共 60 个可审计来源 → 可复现索引 → 139 个实体/398 条显式关系/6 个疾病社区 → 关键词+证据等级+图邻域检索。每个回答依据带来源、章节、证据等级、关系路径、个性化因素、安全标记、一周行动计划和免责声明；索引同时输出版本、SHA256、审核状态和冲突检查结果。
