...
它是课堂演示和局域网试用的轻量实现，不是临床决策系统。当前知识库包含 18 个 WHO、AHA/ASA、ADA、KDIGO、PubMed 随机试验和系统综述摘要，索引含 51 个分块、97 个实体、356 条关系和 6 个疾病社区，并通过 `input/relations.json` 显式维护五类慢病、老年衰弱、危险因素、指标、干预和急症关系。来源、证据等级和更新规则见 `KNOWLEDGE_SOURCES.md`。
...
查询结果现在还会返回 `recommendations`、`graph_paths`、`personalization`、`safety_flags` 和 `citations`：行动由疾病、当前指标上下文和危险信号共同决定，包含 `priority/action/reason/evidence`；智能体会把这些结构化行动和证据强制带入最终回答。`evaluate_graph.py` 和 `evaluate_personalization.py` 用不同用户上下文验证同一问题会得到不同的行动优先级。
