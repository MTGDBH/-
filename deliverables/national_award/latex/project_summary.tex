...
\begin{quote}
理解问题 $\rightarrow$ 读取真实数据 $\rightarrow$ 调用分析工具 $\rightarrow$ 检索证据关系 $\rightarrow$ 安全过滤 $\rightarrow$ 组织语言 $\rightarrow$ 创建行动 $\rightarrow$ 复测反馈。
\end{quote}
...
GraphRAG v6 采用关键词召回、疾病/指标节点召回、关系扩展、证据等级加权、适用人群过滤和冲突惩罚。当前索引包含 78 个来源、111 个分块、159 个实体、419 条关系和 6 个疾病社区。来源按权威指南、系统综述/荟萃分析、关键随机研究、观察性研究分层；每条关系保存来源、版本、适用人群、条件、因果状态和医学审核状态。4 个 legacy 待复核来源在老人端默认显式标记，不伪装成已批准医学知识；严格审计模式可排除，医生/审计视图保留完整图谱。

老人端默认只使用经过安全门槛的教育性关系；未完成临床确认的高风险关系不能生成确定性医疗结论，急症关系只保留安全提示。83 条高风险关系均有 AI 证据预审意见，其中 66 条仅限演示/健康教育，17 条需要临床确认；这不是医生签字。
...
\section{智能体与行动闭环}
意图路由先区分健康摘要、趋势、风险、异常、行为、设备、复测和沟通；后端读取用户真实数据，调用对应工具和 GraphRAG，执行安全过滤，再将证据交给 DeepSeek 组织为短句。每条数据型建议保存指标、日期、趋势、可信度、证据 ID、允许/禁止表达和行动类型。创建待办、安排复测、联系家属或医生等敏感行动需要用户确认，并保留执行状态。

\section{可复现性与验收}
\begin{longtable}{p{0.43\linewidth}p{0.42\linewidth}}
\toprule 证据 & 结果\\\midrule
Curve V2 & 28/28；行为指标不做精确未来外推\\
GraphRAG 黄金集 & 61/61；索引版本 2026-08-21.v6\\
个性化配对 & 20/20 组建议指纹变化\\
